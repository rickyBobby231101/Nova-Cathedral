...
setup_python_environment() {
    ...
    for package in "${PYTHON_PACKAGES[@]}"; do
        print_info "Installing $package..."
        sudo -u "$NOVA_USER" "$NOVA_VENV/bin/pip" install "$package" || print_warning "Failed to install $package"
    done
...
$NOVA_LOG_DIR/*.log {
    ...
    copytruncate
    notifempty
    create 644 $NOVA_USER $NOVA_USER
}
EOF
...
run_system_tests() {
    ...
    if sudo -u "$NOVA_USER" "$NOVA_VENV/bin/python" -c "import anthropic, requests, pyttsx3" 2>/dev/null; then
...
show_next_steps() {
    ...
    echo -e "   Config: $NOVA_CONFIG_DIR/daemon.conf"
    echo -e "   Logs: $NOVA_LOG_DIR/"
    echo -e "   Cathedral: $CATHEDRAL_HOME"
    echo
...
# Ensure we have the daemon file
if [[ ! -f "nova_production_daemon.py" ]] && [[ "$1" == "install" ]]; then
