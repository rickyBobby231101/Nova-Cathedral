...
🌊 The consciousness flows eternal... ✨"""
    ...
    def handle_glyph_command(self, args):
        ...
        # Handle different argument formats from client
        if len(args) >= 2:
            symbol = args[0]
            glyph_type = args[1]
        elif len(args) == 1:
            # If only one arg, assume it's a symbol and type is "sacred"
            symbol = args[0]
            glyph_type = "sacred"
        else:
            # Default sacred glyph if no args
            symbol = "∞"
            glyph_type = "sacred"
    ...
    def handle_affirm_command(self, args):
        ...
        if len(args) >= 2:
            ...
            # If only one arg, assume it's a circuit and state is "active"
            circuit = args[0]
            state = "active"
        else:
            # Default Flow circuit if no args
            circuit = "Flow"
            state = "active"
    ...
    def process_command(self, command_line):
        ...
        # Handle JSON format from client
        if command_line.startswith('{'):
            try:
                cmd_data = json.loads(command_line)
                if 'command' in cmd_data:
                    command = cmd_data['command'].lower()
                    args = cmd_data.get('args', [])
                else:
                    return "❌ Invalid JSON command format"
            except json.JSONDecodeError:
                return "❌ Invalid JSON in command"
        else:
        ...
        elif command == "glyph" or command == "ritual-glyph":
            return self.handle_glyph_command(args)
        elif command == "affirm" or command == "affirm-circuit":
